...
for test_conf in [sdat,ddat,bdat]:
    ...
    class DREstimatorTest(unittest.TestCase):
        ...
        def test_update_step_ignore(self):
            ...
            v_t = torch.tensor(test_conf.test_dm_s_values[0][-1]).squeeze()
            p_t = test_conf.weight_test_res[0,-1]
            r_t = torch.tensor(test_conf.test_reward_values[0][-1]).squeeze()
            q_t = torch.tensor(test_conf.test_dm_sa_values[0][-1]).squeeze()
        ...
        def test_get_traj_discnt_reward(self):
            ...
            def q_side_effect(state:torch.Tensor, action:torch.Tensor):
                ...
                return torch.Tensor(res).squeeze()
            ...
            for idx, traj in enumerate(zip(
                ...
                p_t = torch.masked_select(traj[1], traj[6]>0).squeeze()
                r_t = torch.Tensor(traj[2]).squeeze()
                a_t = torch.Tensor(traj[3])
                q_t = torch.Tensor(traj[4]).squeeze()
                v_t = torch.Tensor(traj[5]).squeeze()
                __pred_res = is_est.get_traj_discnt_reward(
                    reward_array=r_t, discount=gamma, 
                    state_array=s_t, action_array=a_t, weight_array=p_t)
                pred_res.append(__pred_res.numpy().reshape(-1))
                ...
                test_res.append(__test_res_v.numpy().reshape(-1))
        ...
        def test_predict_traj_rewards(self):
            ...
            is_est = DREstimator(dm_model=dm_model, norm_weights=False, clip=0.0, 
                                ignore_nan=True)
            rewards = [torch.Tensor(x) for x in test_conf.test_reward_values]
            ...
            for idx, (r,s,a,w,msk) in enumerate(zip(
                ...
                __test_res = is_est.get_traj_discnt_reward(
                    reward_array=r.squeeze(), discount=gamma, state_array=s, 
                    action_array=a, 
                    weight_array=p)
                test_res.append(__test_res.numpy().reshape(-1))
