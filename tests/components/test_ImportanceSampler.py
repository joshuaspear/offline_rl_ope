...
class TestISWeightCalculator:
    ...
    def __init__(self) -> None:
        self.is_weights = weight_test_res
        self.is_msk = msk_test_res
...
class ISWeightCalculatorTest(unittest.TestCase):
    ...
    def test_get_dataset_w(self):
        ...
        self.assertEqual(is_weights.shape, weight_test_res.shape)
        self.assertEqual(weight_msk.shape, weight_test_res.shape)
        tol = torch.Tensor(self.tollerance).view(-1,1).expand(
            size=(len(self.tollerance), is_weights.shape[1])).mean()
        np.testing.assert_allclose(
            is_weights.numpy(), weight_test_res.numpy(), atol=tol.numpy()
            )
        np.testing.assert_allclose(
            weight_msk.numpy(), msk_test_res.numpy(), atol=tol.numpy()
            )            
